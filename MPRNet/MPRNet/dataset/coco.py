...
class Coco_MO_Train(data.Dataset):
    ...
    def Augmentation(self, image, label,sampled_f_m = None):
        ...
        tx1_n = 0 + tx1 + tx1_ * tx1/(abs(tx1)+1e-5)
        ty1_n = 0 + ty1 + ty1_ * ty1 / (abs(ty1) + 1e-5)
        tx2_n = 0 + tx2 + tx2_ * tx2/(abs(tx2)+1e-5)
        ty2_n = h-1 + ty2 + ty2_ * ty2/(abs(ty2)+1e-5)
        tx3_n = w-1 + tx3 + tx3_ * tx3/(abs(tx3)+1e-5)
        ty3_n = h-1 + ty3 + ty3_ * ty3/(abs(ty3)+1e-5)
        tx4_n = w-1 + tx4 + tx4_ * tx4/(abs(tx4)+1e-5)
        ty4_n = 0 + ty4 + ty4_ * ty4/(abs(ty4)+1e-5)
        src_points = np.float32([[tx1_n,ty1_n],[tx2_n,ty2_n],[tx3_n,ty3_n],[tx4_n,ty4_n]])
        H2,_ = cv2.findHomography(src_points, dst_points, cv2.RANSAC, 10)

        dst_points = np.float32([[0,0],[0,h-1],[w-1,h-1],[w-1,0]])
        tx1__ = random.randint(0,w//10)
        ty1__ = random.randint(0,h//10)
        tx2__ = random.randint(0,w//10)
        ty2__ = random.randint(0,h//10)
        tx3__ = random.randint(0,w//10)
        ty3__ = random.randint(0,h//10)
        tx4__ = random.randint(0,w//10)
        ty4__ = random.randint(0,h//10)
        src_points = np.float32([[tx1_n+tx1__ * tx1/(abs(tx1)+1e-5),ty1__ + ty1_n* ty1/(abs(ty1)+1e-5)],[tx2_n+tx2__ * tx2/(abs(tx2)+1e-5),ty2_n + ty2__ * ty2/(abs(ty2)+1e-5)],
                                 [tx3_n + tx3__ * tx3/(abs(tx3)+1e-5),ty3_n + ty3__ * ty3/(abs(ty3)+1e-5)],[tx4_n + tx4__ * tx4/(abs(tx4)+1e-5),ty4_n + ty4__ * ty4/(abs(ty4)+1e-5)]])
        H3,_ = cv2.findHomography(src_points, dst_points, cv2.RANSAC, 10)
        ...
        image3 = cv2.warpPerspective(image3, H3, (w,h),flags = cv2.INTER_LINEAR)
        label3 = cv2.warpPerspective(label3, H3, (w,h),flags = cv2.INTER_NEAREST)

        ob_loc = ((label + label1 + label2 +label3) > 0).astype(np.uint8)
        ...
        image3 = image3[start_h:end_h, start_w:end_w]
        label3 = label3[start_h:end_h, start_w:end_w]
        ...
        if sampled_f_m != None:
            for sf,sm in sampled_f_m:
                ...
                frame4x = min(383,max(0,random.randint(0,40) + frame3x))
                frame4y = min(383,max(0,random.randint(0,20) + frame3y))
                ...
                sf_c = sf.copy()
                sf_c[sm == 0] = 0
                label3[max(0, frame4y - h // 2):min(384, frame4y + h - h // 2),
                max(0, frame4x - w // 2):min(384, frame4x + w - w // 2)][
                    sm[max(0, h // 2 - frame4y):min(h, h // 2 + 384 - frame4y),
                    max(0, w // 2 - frame4x):min(w, w // 2 + 384 - frame4x)] != 0] = np.max(sm)
                image3[max(0, frame4y - h // 2):min(384, frame4y + h - h // 2),
                max(0, frame4x - w // 2):min(384, frame4x + w - w // 2)][
                    sm[max(0, h // 2 - frame4y):min(h, h // 2 + 384 - frame4y),
                    max(0, w // 2 - frame4x):min(w, w // 2 + 384 - frame4x)] != 0] = 0
                image3[max(0, frame4y - h // 2):min(384, frame4y + h - h // 2),
                max(0, frame4x - w // 2):min(384, frame4x + w - w // 2)] = image3[max(0, frame4y - h // 2):min(384,
                                                                                                               frame4y + h - h // 2),
                                                                           max(0, frame4x - w // 2):min(384,
                                                                                                        frame4x + w - w // 2)] + sf_c[
                                                                                                                                 max(
                                                                                                                                     0,
                                                                                                                                     h // 2 - frame4y):min(
        ...
        return [image,image1,image2,image3], [label,label1,label2,label3]
    ...
    def __getitem__(self, index):
        ...
        N_masks = np.empty((4,)+(384,384,), dtype=np.uint8)
        # frames_ = []
        # masks_ = []
        ...
        h,w,_ = frame.shape
        mask = np.zeros((h,w,25)).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    def get_arguments():
        ...
        parser.add_argument("-o", type=str, help="", default='./test_coco')
        parser.add_argument("-Dcoco", type=str, help="path to coco",default='/data/jialewang/STM/')
        parser.add_argument("-Ddavis", type=str, help="path to davis",default='/data/jialewang/SSM-VOS/datasets/DAVIS2017/')
